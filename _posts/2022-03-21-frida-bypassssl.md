...
- Pin the certificate: The app developer downloads the server certificate, put it in the app bundle and performs comparison at runtime
- Pin the public key: Include the public key in the application bundle as a hash, compares the public-key at runtime with the hash.
...
SSL-pinning in Dart
Dart uses the HttpClient class, namely, its private standard implementation to send http requests. Whether the connection is secure or not is determined based on the addressing scheme of the website.
bool isSecure = (uri.scheme == «https»);
If secure connection is available, SecureSocket will attempt to establish a secure connection. In case of failure we will get notified that the connection uses a compromised certificate if we had previously registered a function that would handle this.
